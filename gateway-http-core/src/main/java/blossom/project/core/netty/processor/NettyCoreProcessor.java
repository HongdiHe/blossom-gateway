...
public class NettyCoreProcessor implements NettyProcessor {
    private static final Logger log = LoggerFactory.getLogger(NettyCoreProcessor.class);
    ...
    public void process(HttpRequestWrapper wrapper) {
        ...
        try {
            ...
            // 通过记录日志并发送内部服务器错误响应处理熔断异常。
            log.error("网关熔断错误，circuit breaker处于open状态", e);
    ...
    /**
     * 关闭 NettyCoreProcessor。(Resilience4j会自动管理资源)
     */
    @Override
    public void shutDown() {
        // Resilience4j会自动管理circuit breaker和其他资源，不需要显式cleanup
    }
}